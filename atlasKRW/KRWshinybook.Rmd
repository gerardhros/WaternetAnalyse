...
EKRlijst1 <- kopDat(meetpuntenAquo, meetpuntenGebied, EKRlijst)
...
EKRset$GHPR<- paste(ifelse(is.na(EKRset$Grootheid.omschrijving)|EKRset$Grootheid.omschrijving == '', EKRset$Typering.Omschrijving, EKRset$Grootheid.omschrijving),ifelse(is.na(EKRset$Parameter.omschrijving)|EKRset$Parameter.omschrijving == '',"",EKRset$Parameter.omschrijving))
...
saveRDS(EKRset, file= paste0("EKRset",gebiedlevel,"2022.rds"))
saveRDS(EKRlijst, file= paste0("EKRlijst",gebiedlevel,"2022.rds"))
}

if(mode == "weergeven"){
source('../scripts/ppr_funs.R')
# inlezen toetsdata
EKRset <- readRDS('../hydrobiologie/EKRsetKRW2022.rds')
# testroutine voor vergelijken toetsjaren in app, volgende 3 regels alleen gebruiken bij testen anders uitcommenten
# EKRset$Waardebepalingsmethode.code <- paste0(EKRset$Waardebepalingsmethode.code,'_2022')
# EKRset2 <- readRDS('../hydrobiologie/EKRsetKRW2020.rds')
# EKRset2$Waardebepalingsmethode.code <- paste0(EKRset2$Waardebepalingsmethode.code,'_2020')

EKRset2 <- readRDS('../hydrobiologie/EKRsetOvWater.rds')
EKRset2$Begindatum <- as.IDate(EKRset2$Begindatum)
EKRset2$Einddatum <- as.IDate(EKRset2$Einddatum)

# KRW doelen 
doelen <- data.table::fread('../hydrobiologie/Doelen.csv')
eag_wl <- data.table::fread('../data/EAG_Opp_kenmerken_20201005.csv')
