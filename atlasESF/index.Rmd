...
ESFoordelen <- read.csv('../data/esfKRW_20200602.csv', header = TRUE, na.strings = " ", sep=";", dec =".", stringsAsFactors = F) # hier later ook eags of gafs aan toevoegen
maatregelen <- read.csv('../data/maatregelenKRW_20200526.csv', header = TRUE, na.strings = " ", sep=";", dec =".", stringsAsFactors = F)# hier later ook eags of gafs aan toevoegen
watertypen <- read.csv('../development/GIS/KRWWatertype.csv', header = TRUE, na.strings = " ", sep=";", dec =".", stringsAsFactors = F)
locaties <- read.csv("../data/location.csv", header = TRUE, na.strings = " ", sep=";", dec =".", stringsAsFactors = F) # hier staat de meest correcte verwijzing naar eags
...
selb <- ppr_slootbodem_kl(bod, wtype = eag_wl, mlocs = locaties)
bod <- ppr_slootbodem(db = bod, wtype = eag_wl, mlocs = locaties)

simoni <- fread("../toxiciteit/overzicht_toxiciteit_2018_2017_2016_2013_2012.csv")
...
#hybi is waterdiepte (1 x per 3 jaar)
# wq is niet representatieve diepte
...
combineWidgets(
    ...
    diepte(hybi[hybi$jaar >= '2015' & hybi$jaar < '2020',])
)
...
```{r}
#maak combinatie slootdiepte + slibdiepte (fill NA when data are missing)
```

In [Grenswaarden waterdiepte] staat de relatie tussen ecologische kwalteit en waterdiepte die gebruikt is voor het bepalen van een optimale waterdiepte per watertype.

```{r}
#maak waarderingskaart op basis van klassegrens (zou je kunnen afleiden) en het handelingsperspectief (waterdiepte + slibdikte > gewenste diepte) voldoende 
# persepectief bestaan 
```

De geringe waterdiepte is de oorzaak van de slechte ecologische toestand in veel lijnvormig water. 

```{r}
# maak een overzichtstabel waar de waterdiepte laag is en het handelingsperspectief (waterdiepte + slibdikte > gewenste diepte) voldoende 
```
...
- Hier ruimte voor tekst en toelichting van de analysematrix en nieuwe figuren die een beter overzicht geven van relevante relaties
...
# bij afleiden klassegrenzen al rekening houden met betrouwbaarheid of geextrapoleerde waterdiepte obv breedte diepte verhouding 
# de relatie tussen breedte en diepteverhouding wordt afgeleid door hybi gegevens te vergelijken met ingemeten diepteprofielen (dwarsdoorsnede)
