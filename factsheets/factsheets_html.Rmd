...
**De huidige toestand vergeleken met de doelen –**`r unique(out$d3$oordeel)`
<br>
De toestand in `r out$waterlichamenwl$OWMNAAM_SGBP3` (zwarte lijnen in de figuur hiernaast) is `r unique(out$d3$oordeel)`. Het slechts scorende biologische kwaliteitselement is `r as.character(unique(out$d3$facet_wrap_code))`. 
...
if(outputF == "html"){  
  plotly::ggplotly(out$plotEKRlijn)%>%
    plotly::layout(legendgroup='GHPR', showlegend = T, legend = list(font = list(size = 9)))
}
if(outputF == "word"){
    out$plotEKRlijn
}
