...
<script>
document.querySelector(".navbar-header > span.navbar-brand").innerHTML = "`r out$my_title`";
</script> 

# Factsheet toestand en ecologische sleutelfactoren (DIPS)

## Hoofdlijnen {data-height=250}

### Beschrijving van het gebied en watersysteem op hoofdlijnen {data-width=600}
Het `r out$waterlichamenwl$OWMNAAM` (`r out$waterlichamenwl$OWL`) heeft watertype `r unique(out$waterlichamenwl$typebesch)` (`r unique(out$eagwl$watertype)`) en bestaat uit de deelgebieden: `r out$deelgebieden$samen`.
<br>
`r out$waterlichamenwl$Karakterschets`
<br>
Onze gebiedspartners zijn provincie `r out$waterlichamenwl$prov` en gemeente(n) `r out$waterlichamenwl$gem`.
Het waterlichaam `r out$waterlichamenwl$OWMNAAM` heeft de status `r out$waterlichamenwl$Status` en is in eigendom van `r out$waterlichamenwl$Eigenaren`. 
...
**De huidige toestand vergeleken met de doelen –**`r out$d3$oordeel`
<br>
De toestand in `r out$waterlichamenwl$OWMNAAM` (zwarte lijnen in de figuur hiernaast) is `r out$d3$oordeel`. Het slechts scorende biologische kwaliteitselement is `r as.character(out$d3$facet_wrap_code)`. 
`r if(!out$d3$facet_wrap_code %in% "Macrofauna"){paste0("De slechts scorende deelmaatlat van dit kwaliteitselement is ",out$d3_deel$GHPR,".")}` 
`r if(!out$d3$facet_wrap_code %in% "Macrofauna"){if(out$d3_deel$GHPR == 'Abundantie groeivormen macrofyten'){paste0("De slechts scorende indicator van deze deelmaatlat is ", out$d3_deelptn$GHPR,".")}}`
...
if(outputF == "word"){
kable(pESFtab, col.names = NULL) %>% kable_styling(full_width=FALSE) %>% 
...
if(outputF == "html"){
kable(pESFtab, format = "html",col.names = NULL) %>% 
...
## Bron {data-height=30}
Deze factsheet is gebaseerd op de KRW toetsing aan (maatlatten 2018) uit 2019, begrenzing waterlichamen 2015-2021, hydrobiologische data 2006-2018 en conceptmaatregelen en doelen voor SGBP3 en `r out$waterlichamenwl$Literatuurverwijzing`.
...
gauge(out$rates$rate_uit, min = 0, max = out$rates$rate_max, symbol = '', gaugeSectors(
...
gauge(out$rates$rate_plan, min = 0, max = out$rates$rate_max, symbol = '', gaugeSectors(
  success = c(80, 100), warning = c(40, 79), danger = c(0, 39)))
```
...
gauge(out$rates$rate_fase, min = 0, max = out$rates$rate_max, symbol = '', gaugeSectors(
  success = c(80, 100), warning = c(40, 79), danger = c(0, 39)))
```
...
gauge(out$rates$rate_del, min = 0, max = out$rates$rate_max, symbol = '', gaugeSectors(
  success = c(80, 100), warning = c(40, 79), danger = c(0, 39)))
```
...
gauge(out$rates$rate_nieuw, min = 0, max = out$rates$rate_max, symbol = '', gaugeSectors(
  success = c(80, 100), warning = c(40, 79), danger = c(0, 39)))
```
...
if(outputF == "word" & nrow(out$maatregelen2)>0){
  knitr::kable(out$maatregelen2) %>% 
          kable_styling(full_width=FALSE) %>% 
          column_spec(1, border_right = F) %>%
...
if(outputF == "html" & nrow(out$maatregelen2)>0){
  knitr::kable(out$maatregelen2, format = "html") %>% 
          kable_styling(full_width=FALSE) %>% 
          column_spec(1, border_right = F) %>%
          column_spec(2, width = "12em", border_right = F)%>%
          collapse_rows(columns=1, valign="top")
...
if(!is.null(out$plotPwbal)){include_graphics(out$plotPwbal)}
...
if(!is.null(out$plotLichtklimaat)){include_graphics(out$plotLichtklimaat)}
if(!is.null(out$plotWaterdiepte)){include_graphics(out$plotWaterdiepte)}
...
if(!is.null(out$plotWaterbodem)){include_graphics(out$plotWaterbodem)}
```
