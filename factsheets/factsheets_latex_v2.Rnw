...
\pagestyle{empty}
\usepackage{helvet}
% set here the compression level for jpg (0 = no compression, 9 = maximum compression)
\pdfcompresslevel4
% use fontfamiliy{vn} for verdana, but its nog working very well, to be checked later
% font phv looks like verdana phv = helvetica (original #1)
\newcommand{\fon}[1]{\fontfamily{phv}\selectfont}
\definecolor{agvblue}{RGB}{52, 152, 219}
\begin{document}
...
lx_litverw <- ifelse(out$waterlichamenwl$Literatuurverwijzing=='','',paste0('en ',out$waterlichamenwl$Literatuurverwijzing))
...
eah_m2 <- if(!out$d3$facet_wrap_code %in% "Macrofauna"){
              if(out$d3_deel$GHPR == 'Abundantie groeivormen macrofyten'){
                  paste0("De slechts scorende indicator van deze deelmaatlat is ", out$d3_deelptn$GHPR[1],".")}} else {NULL}
...
pESFtab[,oms_latex := gsub('<','$<$',gsub('µ','micro',gsub('>','$>$', gsub('&','en', gsub('°',' graden', oms_latex)))))]

# load maatregelentabel, and select the latex columns
cols <- c('ESFoordeel_latex','SGBPPeriode','Naam','Toelichting_latex','BeoogdInitiatiefnemer','UitvoeringIn')
...
maatregelen2[,BeoogdInitiatiefnemer := gsub('Natuurmonumenten|natuurmonumenten','Natuur monumenten',BeoogdInitiatiefnemer)]
maatregelen2[,BeoogdInitiatiefnemer := gsub('Staatsbosbeheer','Staatsbosbeheer',BeoogdInitiatiefnemer)]
#maatregelen2[,SGBPPeriode := gsub('Niet opgenomen in SGBP','Waterschap AGV',Gebiedspartner)]
#maatregelen2[,Gebiedspartner := gsub('Natuurmonumenten|natuurmonumenten','Natuurmonumenten',Gebiedspartner)]
#maatregelen2[,Gebiedspartner := gsub('Staatsbosbeheer','Staatsbosbeheer',Gebiedspartner)]
maatregelen2[,Toelichting_latex := gsub('m³','m3',Toelichting_latex)]
maatregelen2[,Toelichting_latex := gsub('\\/','- en ',gsub('\\r\n',"",Toelichting_latex))]
maatregelen2[,Naam := gsub('\\/','-',Naam)]
...
# reset names
setnames(maatregelen2,c('ESF-oordeel','Periode','Naam','Toelichting','Initiatief','Uitvoering'))
...
@
  ...
  \begin{tcbposter}[
    ...
    colback=white,opacityback=1,colframe=black!5!white,colbacktitle=agvblue,coltitle=white,
    %title style={left color=blue,right color=blue},
    fonttitle=\fon{phv}\bfseries\small\scshape,
    fontupper=\fon{phv}
  }
  ]
  
  % Add the top row with title - fontupper = \fontsize{120}{110}\selectfont \bfseries
  \posterbox[blankest,interior engine=path,height=3cm,halign=center,valign=center,colupper=black,fonttitle=\fon{phv}\bfseries,fontupper=\fon{phv}\scshape]{name=title,column=1,span=8,below=top}{\fontsize{\Sexpr{cf_tit[1]}}{\Sexpr{cf_tit[2]}}\selectfont \Sexpr{lx_title}}
 ...
 % add frame for figuur Ligging waterlichaam (if not saved earlier, use: routput/figure/plot_wlligging-1.pdf)
  \posterbox[adjusted title=Ligging waterlichaam,height = 6cm,valign=center]{name=fig2,column=3,span=2,below=gb}{
     \centering \includegraphics[height = 5cm,keepaspectratio]{\Sexpr{mapEAG}}
  }
  ...
  \posterbox[adjusted title=Ligging deelgebieden,height = 6cm,valign=center]{name=fig3,column=5,span=2,below=gb}{
    \centering \includegraphics[height = 5cm,keepaspectratio]{\Sexpr{mapDEELGEBIED}}
  }
  ...
  \posterbox[adjusted title=Huidige toestand,height = 6cm,valign=center]{name=fig4,column=7,span=2,below=gb}{
    \centering
    \includegraphics[height = 4.5cm,keepaspectratio]{\Sexpr{mapEKR}}
  }

 %add reference methodology information in the bottom
  \posterbox[blankest,interior engine=path,height=0.6cm,fontupper=\fon{phv}\small,colupper=black]{name=ref,column=1,span=8,above=bottom}{
  
   % reduce size of the font
  \fontsize{8}{9.6}\selectfont Auteur: laura.moria@waternet.nl. Deze factsheet is gebaseerd op de KRW toetsing aan (maatlatten 2018) uit 2019, begrenzing waterlichamen 2015-2021, hydrobiologische data 2006-2018 en conceptmaatregelen en doelen voor SGBP3 \Sexpr{lx_litverw}.
    }  
  ...
  \posterbox[adjusted title=Ecologische analyse op Hoofdlijnen]{name=analyse,column=1,span=8,between=fig1 and esf}{
  ...
  \posterbox[blankest,interior engine=path,height=3.2cm,halign=center,valign=center,colupper=black,fonttitle=\fon{phv}\bfseries,fontupper=\fon{phv}\scshape, fontlower =\fon{phv}\itshape ]{name=title,column=1,span=10,below=top}{
...
if(nrow(maatregelen2)>4){
  ...
  # text size of the toelichting column
  tsize = sum(nchar(out$maatregelen2$Toelichting_latex))
  cf = 0
  # make special correction factor for some factsheets
  if(grepl('botshol',tolower(out$my_title2))){cf = 3900 / tsize}
  if(grepl('bovenke',tolower(out$my_title2))){cf = 4700 / tsize}
  if(grepl('noorderleg',tolower(out$my_title2))){cf = 4300 / tsize}
  if(grepl('vinkeve',tolower(out$my_title2))){cf = 7400 / tsize}
  if(grepl('sloterplas',tolower(out$my_title2))){cf = 4200 / tsize}
  if(grepl('noorderijplas',tolower(out$my_title2))){cf = 2800 / tsize} 
  if(grepl('gaasper',tolower(out$my_title2))){cf = 3500 / tsize} 
  if(grepl('hethol',tolower(out$my_title2))){cf = 5600 / tsize}
  if(grepl('loenderveenoost',tolower(out$my_title2))){cf = 4000 / tsize}
  if(grepl('ouderkerkerplas',tolower(out$my_title2))){cf = 6500 / tsize} 
  if(grepl('loosdrechtseplas1tm5',tolower(out$my_title2))){cf = 8100 / tsize} 
  if(grepl('kievitsbuurt',tolower(out$my_title2))){cf = 5800 / tsize}
  #if(grepl('amstellandboezem',tolower(out$my_title2))){cf = 5820 / tsize}
  if(grepl('^vecht$',tolower(out$my_title2))){cf = 5400 / tsize}
  if(grepl('vaartenamsterdam',tolower(out$my_title2))){cf = 4000 / tsize}
  if(grepl('vaartenvechtstreek',tolower(out$my_title2))){cf = 5600 / tsize}
  if(grepl('vaartengrootmijdrecht',tolower(out$my_title2))){cf = 2500 / tsize}
  if(grepl('spiegelplas',tolower(out$my_title2))){cf = 6000 / tsize}
  if(grepl('wijdeblik',tolower(out$my_title2))){cf = 4000 / tsize}
  if(grepl('waterleidingplas',tolower(out$my_title2))){cf = 2000 / tsize}
  if(grepl('naardermeer',tolower(out$my_title2))){cf = 3800 / tsize}
  if(grepl('molenpolder',tolower(out$my_title2))){cf = 5900 / tsize}
  if(grepl('noorderijplas',tolower(out$my_title2))){cf = 3300 / tsize}
  if(grepl('ouderkerkerplas',tolower(out$my_title2))){cf = 4400 / tsize}
  if(grepl('breukeleveenscheplas',tolower(out$my_title2))){cf = 5500 / tsize}
  if(grepl('tienhovenseplassen',tolower(out$my_title2))){cf = 6200 / tsize}
  if(grepl('sterenzodden',tolower(out$my_title2))){cf = 7200 / tsize}
  if(grepl('hollandsankeveenseplassen',tolower(out$my_title2))){cf = 6500 / tsize}
  if(grepl('wijdegat',tolower(out$my_title2))){cf = 5700 / tsize}
  if(grepl('hilversumskanaal',tolower(out$my_title2))){cf = 3000 / tsize}
  if(grepl('oostelijkebinnenpolder',tolower(out$my_title2))){cf = 5300 / tsize}
  if(grepl('tussenboezemvinkeveena',tolower(out$my_title2))){cf = 3500 / tsize}
  if(grepl('mijdrechtsebovenlanden',tolower(out$my_title2))){cf = 4300 / tsize}
  if(grepl('maarsseveensezoddenenomgeving',tolower(out$my_title2))){cf = 3700 / tsize}
  if(grepl('grootwilnis-vinkeveenzuid',tolower(out$my_title2))){cf = 4400 / tsize}
  if(grepl('demmerik',tolower(out$my_title2))){cf = 4400 / tsize}
  if(grepl('vaartenzevenhoven',tolower(out$my_title2))){cf = 4400 / tsize}
  if(grepl('vinkeveenseplassen',tolower(out$my_title2))){cf = 6000 / tsize}
  if(grepl('grotemaarsseveenseplas',tolower(out$my_title2))){cf = 3500 / tsize}
  
  if(cf==0) {cf = 4900 / tsize}
...
# make table
knitr::kable(maatregelen2[,1:6], format = "latex",align = 'l',escape = FALSE) %>% 
          kable_styling(full_width=FALSE, position = "left",latex_options = "scale_down") %>% 
          column_spec(1, width = "1cm", border_right = F,color = "black") %>%          
          column_spec(2, width = "2cm", border_right = F,color = "black") %>%        
          column_spec(3, width = "4cm",border_right = F,color = "black") %>%
          column_spec(4, width = "15cm",border_right = F,color = "black") %>%
          column_spec(5, width = "3cm", border_right = F,color = "black")  %>%
          column_spec(6, width = "1.5cm", border_right = F,color = "black")  %>%
...
@
 ...
 %add reference methodology information in the bottom
  \posterbox[blankest,interior engine=path,height=0.6cm,fontupper=\fon{phv}\small,colupper=black]{name=ref,column=1,span=10,above=bottom}{
  
   % reduce size of the font
  \fontsize{8}{9.6}\selectfont Auteur: Laura Moria, laura.moria@waternet.nl. Deze factsheet is gebaseerd op de KRW toetsing aan (maatlatten 2018) uit 2020, begrenzing waterlichamen 2020, hydrobiologische data 2006-2019 en conceptmaatregelen en doelen voor SGBP3 \Sexpr{lx_litverw}. De R- en LaTeX-code om deze pdf's te genereren is ontwikkeld door Gerard H. Ros, gerard.ros@waternet.nl.
    }  
  ...
  \begin{tcbposter}[
    ...
    % Add the top row with title
  \posterbox[blankest,interior engine=path,height=3.2cm,halign=center,valign=center,colupper=black,fonttitle=\fon{phv}\bfseries,fontupper=\fon{phv}\scshape, fontlower =\fon{phv}\itshape ]{name=title,column=1,span=6,below=top}{
  \fontsize{\Sexpr{cf_tit[3]}}{\Sexpr{cf_tit[3]}}\selectfont \Sexpr{lx_title}\\[3mm]
  \fontsize{18}{16.5}\selectfont
  ...
  \posterbox[adjusted title=Motivering KRW status en herbegrenzing,valign=top,equal height group=A]{name=krwstatus,column=1,span=3,below=title}{
    \vspace{5mm}
    \Sexpr{lx_motstat}
    \vspace{5mm}
  }
  ...
  \posterbox[adjusted title=Monitoringswensen,valign=top,equal height group=A]{name=monwens,column=4,span=3,below=title}{
  \vspace{5mm}
  \Sexpr{lx_monwens}
  \vspace{5mm}
  } 
  ...
  \posterbox[adjusted title=ESF1 productiviteit,valign=center,height = 10cm]{name=p3fig1,column=1,span=2,below=monwens}{
  \centering
  ...
  \posterbox[adjusted title=ESF2 lichtklimaat,valign=center,height = 10cm]{name=p3fig2,column=3,span=2,below=monwens}{
  \centering
  ...
  \posterbox[adjusted title=ESF4 Waterdiepte,valign=center,height = 10cm]{name=p3fig3,column=5,span=2,below=monwens}{
  \centering
  ...
  \posterbox[adjusted title=ESF1 en 3 waterbodem,valign=center,height = 10cm]{name=p3fig4,column=1,span=2,below=p3fig1}{
  \centering
   ...
   % add frame for ESF1 en 3 waterbodem
  \posterbox[adjusted title=ESF1 en 3 waterbodem,valign=center,height = 10cm]{name=p3fig5,column=3,span=2,below=p3fig1}{
  \centering
  ...
  %add reference methodology information in the bottom
  \posterbox[blankest,interior engine=path,height=0.6cm,fontupper=\fon{phv}\small,colupper=black]{name=ref,column=1,span=6,above=bottom}{
  
   % reduce size of the font
  \fontsize{8}{9.6}\selectfont Auteur: Laura Moria, laura.moria@waternet.nl. Deze factsheet is gebaseerd op de KRW toetsing aan (maatlatten 2018) uit 2019, begrenzing waterlichamen 2015-2021, hydrobiologische data 2006-2018 en conceptmaatregelen en doelen voor SGBP3 \Sexpr{lx_litverw}. De R- en LaTeX-code om deze pdf's te genereren is ontwikkeld door Gerard H. Ros, gerard.ros@waternet.nl.
    }  
  \end{tcbposter}
