...
ESFoordelen <- fread('../data/esfKRW_20200228.csv') # hier later ook eags of gafs aan toevoegen
ESFoordelen[,11:25] <- ESFoordelen[,lapply(.SD, function(x) trim(x)),.SDcols = 11:25]
maatregelen <- fread('../data/maatregelenKRW_20200228.csv')# hier later ook eags of gafs aan toevoegen
...
maatregelen[,2:25] <- maatregelen[,lapply(.SD, function(x) trim(x)),.SDcols = 2:25]
...
for (i in 11:17){
  #nrow(ESFoordelen)
    ...
    if(nrow(eag_wl[eag_wl$KRW_SGBP3 %in% wl,])>0){ 
      eagwl <- eag_wl[eag_wl$KRW_SGBP3 %in% wl|substr(eag_wl$GAFIDENT,1,4) %in% wl,]
    ...
    if(nrow(eag_wl[eag_wl$KRW_SGBP3 %in% wl,])==0){
      ...
      wlname <- unique(EKRset$HoortBijGeoobject.identificatie[EKRset$EAGIDENT %in% wl])
      wlname <- gsub("NL11_","",wlname)
      } 

    my_title <- paste0(waterlichamenwl$OWMNAAM_SGBP3)
    # als wl is waterlichaam dan worden balansen die bij wl horen volgens koppeltabel, ook eagbalansen worden geslecteerd
...
if(dtEST2$KRWWaterlichaamcode %in% c('NL11_1_2')){
  myimages<- "esticon/W5_O1_K_L.jpg" 
}
...
maatregelen2 <- maatregelen2[,c(12,7,5,6,8,9,10,11)] # weer te geven kolommen selecteren
