...
# inladen hydrobiologische gegevens -----------
  ...
  # update, filter and clean up databases -----------
    ...
    #EKRset$KRWwatertype.code <- EKRset$KRWwatertype.code.y
...
#write.table(pvskp, file = paste("PvskPditchlakeALL",format(Sys.time(),"%Y%m%d%H%M"),".csv", sep= ""), quote = FALSE, na = "", sep =';', row.names = FALSE)
...
for (i in 8:nrow(ESFoordelen)){
    #i=13 vaarten vechtstreek: meer ruimte voor karakterschets
    ...
    waterlichamenwl$typebesch <- tolower(watertypen$Omschrijving[watertypen$Code %in% unique(eagwl$watertype)])
    ...
    EST_sel <- EST[EST$Waterlichaam %in% eagwl$GAFIDENT,]
...
for (i in 11:50){
#nrow(ESFoordelen)
     #i=10 amstelland loopt vast pandoc error 99
    # i=30 loopt vast 
  #  waterlichamenwl <- ESFoordelen[i,] # i als iteratie aan, anders nummer waterlichaam
    wl <- waterlichamenwl$OWL_SGBP3 # of waterlichaam of eagcode of gafcode
    # als eenheid factsheet KRW wl of GAF of meerdere EAGs
    ...
    if(nrow(eag_wl[eag_wl$KRW_SGBP3 %in% wl,])>0){ 
      eagwl <- eag_wl[eag_wl$KRW_SGBP3 %in% wl %in% wl,]
      wlname <- unique(eagwl$KRWmonitoringslocatie_SGBP3[(eagwl$KRW_SGBP3 %in% wl)])
      wlname <- sapply(strsplit(wlname, '_'), `[`, 2) # hoortbijgeo zonder namespace oor selectie van totaalscores per waterlichaam
    } 
    # als eenheid factsheet is eag of gaf
    if(nrow(eag_wl[eag_wl$KRW_SGBP3 %in% wl,])==0){
      eagwl <- eag_wl[eag_wl$GAFIDENT %in% wl|substr(eag_wl$GAFIDENT,1,4),]
      wlname <- unique(EKRset$HoortBijGeoobject.identificatie[EKRset$EAGIDENT %in% eagwl$GAFIDENT])
      wlname <- gsub("NL11_","",wlname)
      } 
...
ekr_scores_sel <- ekr_scores[is.na(ekr_scores$EAGIDENT) & !is.na(ekr_scores$id),]
## make neat titles, alleen hoofdmaatlatten
ekr_scores_sel$facet_wrap_code <- as.factor(mapvalues(ekr_scores_sel$wbmethode, 
...
ekr_scores_sel2 <- ekr_scores_sel[!ekr_scores_sel$wbmethode %in% c("Maatlatten2012 Ov. waterflora","Maatlatten2012 Vis") & ekr_scores_sel$level == 1,]
...
## Hoofdlijnen {data-height=265}

### Beschrijving van het gebied en watersysteem op hoofdlijnen {data-width=650}
`r waterlichamenwl$OWMNAAM` (`r waterlichamenwl$OWL`) heeft watertype "`r unique(waterlichamenwl$typebesch)`" (`r unique(eagwl$watertype)`) en het wateroppervlak van het waterlichaam is `r round(sum(eagwl$OppWater)/10000, 0)` hectare.
<br>
...
dtEST2 <- merge(EST_sel, eagwl, by.x = "Waterlichaam", by.y  = "GAFIDENT")
...
if(dtEST2$KRW_SGBP3 %in% c('NL11_3_8')){
 myimages<- "esticon/W6_O7_DM_L.jpg" 
}
if(dtEST2$KRW_SGBP3 %in% c('NL11_5_1')){
  myimages<- "esticon/W4_O6_OM_L.jpg" 
}
if(dtEST2$KRW_SGBP3 %in% c('NL11_1_2')){
  myimages<- "esticon/W6_O6_K_St.jpg" 
}
if(dtEST2$KRW_SGBP3 %in% c('NL11_1_1')){
  myimages<- "esticon/W6_O6_K_St.jpg" 
}
...
## make neat titles, alleen hoofdmaatlatten
ekr_scores_sel2_deel$facet_wrap_code <- as.factor(mapvalues(ekr_scores_sel2_deel$wbmethode, 
...
if(length(unique(ekr_scores_sel2$id))==1){
ekrplot2(ekr_scores_sel2)
}

if(length(unique(ekr_scores_sel2$id))>1){
a = split(ekr_scores_sel2,list(ekr_scores_sel2$id), drop=TRUE)

do.call(grid.arrange,c(lapply(1:length(a), function(i) ekrplot2(as.data.frame(do.call('rbind',a[i])))), nrow=1))
}
```
...
### EKR scores op alle deelmaatlatten in de tijd {data-height=800}
