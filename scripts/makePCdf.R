...
#        filter(jaar > 2008) %>%  
#        group_by(locatie.EAG,jaar,maand,fewsparameter) %>%
#        summarise_at(c('meetwaarde'), mean, na.rm = TRUE)
#   wq2 <- wq %>%
#        filter(fewsparameter %in% wqparameter)%>%
#        filter(jaar > 2008) %>%  
#        group_by(locatie.afaanvoergebied,jaar,maand,fewsparameter) %>%
#        summarise_at(c('meetwaarde'), mean, na.rm = TRUE)
# ext <- rbind(wq1,wq2)
# write.table(ext, file = paste(getwd(),"/extinctie",format(Sys.time(),"%Y%m%d%H%M"),".csv", sep= ""), quote = FALSE, na = "", sep =';', row.names = FALSE)


#bod_ondiep <- bod[!(bod$fewsparameter == "MONSDTE_m_E" & bod$meetwaarde < -10),]
#selb <- dcast(bod_ondiep, locatie.EAG+locatie.afaanvoergebied+datum+jaar ~ fewsparameter+compartiment, value.var = "meetwaarde", fun.aggregate = mean)
#write.table(selb, file = paste(getwd(),"/waterbodemchemie",format(Sys.time(),"%Y%m%d%H%M"),".csv", sep= ""), quote = FALSE, na = "", sep =';', row.names = FALSE)
...
dg <- dat %>%
  ...
  # groeperingsfactor
  group_by(pol, EAG, GAF, KRW, watertype, bodem, jaar, maand,
...
for (i in unique(nomogram$bodemtype)){
  ...
  # j <- unique(nomogram_bt$watdteF)[1]
...
# bereken kP ------------------------------------------------------------
PvskP$PvskPDitch<- PvskP$wp_tot_sum/PvskP$kP # >1 is te veel

# koppel kp plassen obv invoertabel per EAG
Overzicht_kP_plas <- Overzicht_kP

sel <- dgwatdte$watertype %in% c('M20','M27','M25',"M30","M14","M11","M6a","M7b","M6b","M7a")  
PvskPplas <-  merge(dgwatdte[sel ,], Overzicht_kP_plas[,c('EAG',"Strijklengte..m.","Diepte_50P..m.","P.load_year..mgP.m2.d.", 'Troebel.naar.helder..mg.P.m2.d.', 
                                                        'Helder.naar.troebel..mg.P.m2.d.', 'lake.ditch.vollenweider')],
                    by.x = 'EAG', by.y = 'EAG', all = F)
# PvskPplas$wp_tot_sum[!is.na(PvskPplas$P.load_year..mgP.m2.d.)]  <- PvskPplas$P.load_year..mgP.m2.d.[!is.na(PvskPplas$P.load_year..mgP.m2.d.)] 
# sel <- PvskPplas$lake.ditch.vollenweider == 'vollenweider' & !is.na(PvskPplas$lake.ditch.vollenweider)
PvskPplas$PvskPLake <- PvskPplas$wp_tot_sum/PvskPplas$Helder.naar.troebel..mg.P.m2.d. # >1 is te veel
PvskPplas1 <-  merge(dgwatdte[sel,], Overzicht_kP_plas[,c('EAG',"P.load_year..mgP.m2.d.", 'Troebel.naar.helder..mg.P.m2.d.', 
                                                         'Helder.naar.troebel..mg.P.m2.d.', 'lake.ditch.vollenweider')],
                     by.x = 'GAF', by.y = 'EAG', all = FALSE)
PvskPplas1$wp_tot_sum[is.na(PvskPplas1$wp_tot_sum)] <- PvskPplas1$P.load_year..mgP.m2.d.[is.na(PvskPplas1$wp_tot_sum)] 
PvskPplas1$PvskPLake <- PvskPplas1$wp_tot_sum/PvskPplas1$Helder.naar.troebel..mg.P.m2.d. # >1 is te veel

PvskPplas2 <-  merge(dgwatdte[sel,], Overzicht_kP_plas[,c('EAG',"P.load_year..mgP.m2.d.", 'Troebel.naar.helder..mg.P.m2.d.', 
                                                         'Helder.naar.troebel..mg.P.m2.d.', 'lake.ditch.vollenweider')],
                     by.x = 'KRW', by.y = 'EAG', all = FALSE)
PvskPplas2$wp_tot_sum [is.na(PvskPplas2$wp_tot_sum )] <- PvskPplas2$P.load_year..mgP.m2.d.[is.na(PvskPplas2$wp_tot_sum )] 
# sel <- PvskPplas$lake.ditch.vollenweider == 'vollenweider' & !is.na(PvskPplas$lake.ditch.vollenweider)
PvskPplas2$PvskPLake <- PvskPplas2$wp_tot_sum /PvskPplas2$Helder.naar.troebel..mg.P.m2.d. # >1 is te veel

pvskp <- smartbind(PvskPplas, PvskPplas1, PvskPplas2)
PvskP <- merge(PvskP, pvskp[,c('pol','EAG',"P.load_year..mgP.m2.d.", 'Troebel.naar.helder..mg.P.m2.d.', 
                            'Helder.naar.troebel..mg.P.m2.d.', 'lake.ditch.vollenweider','PvskPLake')], by = c('pol','EAG'), all.x = TRUE, all.y =FALSE)
return(PvskP)
write.table(PvskP, file = paste(getwd(),"/pbelasting/output/PvskPditchlake_ALL",format(Sys.time(),"%Y%m%d%H%M"),".csv", sep= ""), quote = FALSE, na = "", sep =';', row.names = FALSE)
saveRDS(PvskP, file = paste0('pbelasting/PvskP','.rds'))
  ...
  map4 <- sp::merge(gGAF, ditch[, c('pol', 'EAG','GAF','KRW', 'bodem','watdte','w_debiet', 'wp_min_sum','wp_inc_sum', 'wp_tot_sum',
   ...
   leaflet() %>%
     addPolygons(data = map4, layerId = map4$GAFIDENT, popup= paste("GAF naam", map4$GAFNAAM, "<br>",
                                                                    "P belasting:", map4$wp_tot_sum , "<br>",
     ...
     addPolygons(data = map, layerId = map$GAFIDENT, popup= paste("EAG naam", map$GAFNAAM, "<br>",
      ...
      addPolygons(data= map2, layerId = map2$GAFIDENT, popup= paste("EAG naam", map2$GAFNAAM, "<br>",
                                                                   "P belasting invoertabel:", map2$P.load_year..mgP.m2.d. , "<br>",
